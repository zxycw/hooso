...
function renderCartPage() {
  ...
  container.innerHTML = cart.map((item, i) => {
    ...
    const logoLabel = item.logoText
      ? ` · LOGO：${item.logoText}`
      : (item.skipLogo ? ' · 不需要LOGO' : '');
    const nfcLabel = item.nfcTiming === 'later'
      ? ' · NFC稍後自行設定'
      : (item.nfcUrl ? ` · NFC：${item.nfcUrl}` : '');
    return `
    <div class="cart-item">
      <div class="cart-item-image"><i class="ti ti-bottle" aria-hidden="true"></i></div>
      <div>
        <div class="cart-item-name">hoo'so ${variantLabel}</div>
        <div class="cart-item-meta">${colorLabel}${logoLabel}${nfcLabel}</div>
